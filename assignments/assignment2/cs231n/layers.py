...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (x, x_hat, mean, var, eps, gamma, out)
        ...
        # scale and shift the normalised data
        out = gamma * x_hat + beta
...
def batchnorm_backward(dout, cache):
    ...
    x, x_hat, mean, var, eps, gamma, out = cache
...
def max_pool_forward_naive(x, pool_param):
    ...
    H_prime = round(1 + (H - pool_height) / stride)
    W_prime = round(1 + (W - pool_width) / stride)
        ...
        for c in range(C):
            for h in range(H_prime):
                for w in range(W_prime):
                    ws = w * stride
                    hs = h * stride
                    out[n, c, h, w] = np.max(x[n, c, hs : hs + pool_height, ws : ws + pool_width])
        ...
        for c in range(C):
            for h in range(H_prime):
                for w in range(W_prime):
                    ws = w * stride
                    hs = h * stride
                    ind = np.argmax(x[n, c, hs : hs + pool_height, ws : ws + pool_width])
                    ind1, ind2 = np.unravel_index(ind, (pool_height, pool_width))
                    dx[n, c, hs : hs + pool_height, ws : ws + pool_width][ind1, ind2] = dout[n, c, h, w]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
